...
`source/content/media/kda/kda-future-animation.html` is the standalone canvas cover, styled after the PTQ dispatch-board animation. Two rows of 16 vector-like blocks surround an orange current-position pivot. The pivot aligns with the selected token column, highlighted in both rows. Selection starts at token 8 and advances automatically with a short glide; clicking a column or focusing the keyboard slider holds a manual selection. Arrow keys and Home/End select tokens. Arrows represent reference lookups: green toward earlier positions, red toward later ones, and orange for the selected position. This is conceptual, not measured data. The figure has no visible wording or caption; an accessible canvas description and an icon-only pause control remain. It uses a `doc-widget widget-frame` iframe, follows the blog theme, and starts paused for reduced-motion preferences. The rejected static SVG cover and its generator were removed.
